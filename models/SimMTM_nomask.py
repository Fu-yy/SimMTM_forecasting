...
class Diffusion(nn.Module):
    def __init__(
        ...
        self.gamma = self.gamma * gamma_rate
        print("self.gamma")
        print(self.gamma_rate)
        # c =self.gamma*0.01
        # self.gamma = c
    ...
    def forward(self, x,rate=1):
        ...
        noisy_x, noise = self.noise(x, t,rate)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.difuss_layers = nn.ModuleList([
            Diffusion(
                time_steps=configs.time_steps,
                device=self.device,
                scheduler='cosine',
                gamma_rate=(1 / self.configs.positive_nums) * (i + 1),
            )
            for i in range(self.configs.positive_nums)
        ])
    ...
    def pretrain(self, x_enc, x_mark_enc, batch_x, mask):
        ...
        noise_list.append(x_enc)
        for layer in self.difuss_layers:
            noise_x, _, _ = layer(
                x_enc
            )
            noise_list.append(noise_x)
        x_enc = torch.cat(noise_list, dim=0)
        bs, seq_len, n_vars = x_enc.shape
        ...
        loss_cl, similarity_matrix, logits, positives_mask = self.contrastive(
            s_enc_out)  # similarity_matrix: [(bs * n_vars) x (bs * n_vars)]  对比损失得到相似度
        rebuild_weight_matrix, agg_enc_out = self.aggregation(similarity_matrix,
                                                              p_enc_out)  # agg_enc_out: [(bs * n_vars) x seq_len x d_model]   根据相似度矩阵对噪声数据重建
...
if __name__ == '__main__':
    ...
    x_mark_enc= torch.randn(16,96,4)
    x_res= torch.randn(16,96,7)
