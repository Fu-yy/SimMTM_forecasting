...
class Exp_SimMTM(Exp_Basic):
    ...
    def pretrain_one_epoch(self, train_loader, model_optim, model_scheduler):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
            ...
            # data augumentation
            batch_x_m, batch_x_mark_m, mask = masked_data(batch_x, batch_x_mark, self.args.mask_rate, self.args.lm, self.args.positive_nums) #batch_x 16 336 7, batch_x_mark 16 336 4   res ==> batch_x_m 48 336 7, batch_x_mark_m 48 336 4, mask 48 336 7
            batch_x_om = torch.cat([batch_x, batch_x_m], 0) # 原始和遮盖后的数据  64 336 7

            batch_x = batch_x.float().to(self.device)  # 16 336 7
            ...
            # encoder
            loss, loss_cl, loss_rb, _, _, _, _ = self.model(batch_x_om, batch_x_mark, batch_x, mask=mask_om)#batch_x_om 原始和遮盖后的数据 batch_x_mark batch_x原始 mask_om遮蔽的和全1   由于batch_x_om是由x和遮蔽后的cat在一起的，所以mask_om全1对应的是x原来的数据

            # batch_x_om 64 336 7 ; batch_x_mark 16 336 4 ； batch_x 16 336 7
    ...
    def pretrain_one_epoch(self, train_loader, model_optim, model_scheduler):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
            ...
            # encoder
            loss, loss_cl, loss_rb, _, _, _, _ = self.model(batch_x, batch_x_mark, batch_x, mask=None)#batch_x_om 原始和遮盖后的数据 batch_x_mark batch_x原始 mask_om遮蔽的和全1   由于batch_x_om是由x和遮蔽后的cat在一起的，所以mask_om全1对应的是x原来的数据
    ...
    def valid_one_epoch(self, vali_loader):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
            ...
            # encoder
            loss, loss_cl, loss_rb, _, _, _, _ = self.model(batch_x, batch_x_mark, batch_x, mask=None)
